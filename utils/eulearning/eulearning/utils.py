...
def vectorize_st(st, filtrations=None):
	...
	Output:
		vec_st			: ndarray of size (st.num_simplices(), len(filtrations)+2) where for each simplex, 
		the line is of the form [(-1)**dim(simplex), filtration value of simplex in st, filtration value of simplex as determined by ndarrays of filtrations].
		If an array of filtrations is of shape (st.num_vertices(),), then the filtration value of each simplex is computed by upper star filtration. 
		Otherwise, this array already indicates the filtration values for each simplex of st. 
		CAUTION: vertices (or simplices) and their values must have the same order in the arrays of filtrations and in st.
	'''
